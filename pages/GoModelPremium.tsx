...
export const GoModelPremium: React.FC = () => {
    const { t } = useLanguage();
            ...
            try {
                ...
                if (auth.currentUser && res.imageUrl) {
                    await saveToHistory(auth.currentUser.uid, {
                        ...
                        type: "Go Model PREMIUM",
                        prompt: `Premium Fashion Photoshoot - ${selectedEthnicity} ${gender}`
                    });
                }
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goModelPremium.page.title')}
                description={t('goModelPremium.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[420px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <section>
                        <StepHeader step={1} title={t('goModelPremium.sections.upload.title')} description={t('goModelPremium.sections.upload.description')} />
                    ...
                    <section className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader step={2} title={t('goModelPremium.sections.options.title')} description={t('goModelPremium.sections.options.description')} />
                    ...
                    <div className="pt-4">
                        <button onClick={handleGenerate} disabled={isGenerating || !productImage} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-2xl shadow-lg transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3">
                            {isGenerating ? <><Spinner className="h-5 w-5 text-white" /> Rendering ({progress}/4)...</> : <><Camera className="w-5 h-5"/> {t('goModelPremium.generateButton')}</>}
                        </button>
                ...
                <section className="flex-1 bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[600px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase italic">Premium Model Gallery</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">4 Variasi model premium mengenakan produk Anda.</p>
                        </div>
                    </div>
                                        ...
                                        <>
                                            ...
                                            <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-end justify-center p-4">
                                                <div className="flex gap-2 w-full">
                                                    ...
                                                    <a href={result.imageUrl!} download={`premium-model-${index+1}.png`} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white p-2.5 rounded-xl transition-all shadow-lg flex items-center justify-center"><DownloadIcon className="w-5 h-5" /></a>
                                                </div>
                                            </div>
                                        </>
                                    )}
