...
const ANGLE_OPTIONS: { id: Angle; nameKey: string; prompt: string; }[] = [
    { id: 'eye-level', nameKey: 'goSepatuV2.angles.eyeLevel', prompt: 'Eye-level shot,' },
    { id: 'high-angle', nameKey: 'goSepatuV2.angles.highAngle', prompt: 'Top view composition from directly above,' },
    { id: 'close-up', nameKey: 'goSepatuV2.angles.closeUp', prompt: 'Detailed close-up shot,' },
    { id: 'detail', nameKey: 'goSepatuV2.angles.detail', prompt: 'Extreme macro photography, focusing on fine textures and shiny details,' },
    { id: 'dutch-angle', nameKey: 'goSepatuV2.angles.dutchAngle', prompt: 'Dutch angle shot,' }
];
...
const getSepatuV2Prompts = () => [
    `Professional product photography of the EXACT shoe/sandal from the source image. The setting is a grey textured floor. One sandal is worn on a right foot, while the left foot is bare and placed casually next to it. Another sandal is placed on the floor at a diagonal angle, facing the feet. Top view composition showing both feet and sandals clearly. Warm indoor lighting makes the product colors look soft and elegant, with shiny details standing out. Natural and casual lifestyle vibe. 4K quality.`,
    `A high-end lifestyle catalog photo for the EXACT shoe/sandal from the source image. The product is displayed on a textured grey floor. One sandal is on a foot, the other is placed diagonally on the floor. The other foot is bare. Top view angle from above. The lighting is warm and clear, making the product's details and textures prominent. The background is a simple grey floor, creating a professional look. Sharp textures and vivid colors.`,
    `Cinematic product showcase of the EXACT shoe/sandal from the source image. The scene features a grey textured floor. A person is wearing one shoe on their right foot, with the left foot bare. The second shoe is placed on the floor at a diagonal angle. Top-down perspective. Natural and casual lifestyle arrangement. The product's intricate details are highly visible due to warm studio lighting. Simple background provides a striking contrast. Ultra-sharp 4K resolution.`,
    `A professional photoshoot of the EXACT shoe/sandal from the source image on a grey textured surface. Lifestyle composition: one shoe on a foot, the other placed on the floor, one foot bare. Top view. Warm indoor lighting ensures all details are prominent and elegant. The grey floor background makes the product stand out. Casual and natural atmosphere, capturing the essence of the design with high-fidelity textures.`
];

export const GoSepatuV2: React.FC = () => {
    const { t } = useLanguage();
    ...
    const handleGenerate = async () => {
        if (!sourceImage) {
            setError(t('goSepatuV2.errors.noImage'));
            return;
        }
        ...
        const prompts = getSepatuV2Prompts();
            ...
            try {
                ...
                if (auth.currentUser && res.imageUrl) {
                    await saveToHistory(auth.currentUser.uid, {
                        ...
                        type: "Go Sepatu v2",
                        prompt: `Go Sepatu v2 - ${angle} angle`
                    });
                }
            } catch (err: any) {
    ...
    const handleDownload = (url: string, index: number) => {
        ...
        link.href = url;
        link.download = `go-sepatu-v2-${index + 1}-${Date.now()}.png`;
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goSepatuV2.page.title')}
                description={t('goSepatuV2.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                     ...
                     <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader 
                            step={3} 
                            title={t('goSepatuV2.sections.angle.title')}
                            description={t('goSepatuV2.sections.angle.description')}
                        />
                        <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="pt-4">
                        ...
                        >
                            {isGenerating ? (
                                ...
                                <span className="tracking-wide">{t('goSepatuV2.generateButton')}</span>
                            )}
                        </button>
                ...
                <section className="flex-1 w-full bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[500px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Go Sepatu v2 Gallery</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">
                                {isGenerating ? `Mengerjakan...` : `4 Variasi Foto Sepatu & Sandal Lifestyle`}
                            </p>
                        </div>
                        {isGenerating && (
