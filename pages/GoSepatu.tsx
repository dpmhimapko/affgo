...
const ANGLE_OPTIONS: { id: Angle; nameKey: string; prompt: string; }[] = [
    { id: 'eye-level', nameKey: 'goSepatu.angles.eyeLevel', prompt: 'Eye-level shot,' },
    { id: 'high-angle', nameKey: 'goSepatu.angles.highAngle', prompt: 'Top view or semi-top view composition,' },
    { id: 'close-up', nameKey: 'goSepatu.angles.closeUp', prompt: 'Detailed close-up shot,' },
    { id: 'detail', nameKey: 'goSepatu.angles.detail', prompt: 'Extreme macro photography, focusing on fine textures, small ornaments like flowers, ribbons, or characters,' },
    { id: 'dutch-angle', nameKey: 'goSepatu.angles.dutchAngle', prompt: 'Dutch angle shot,' }
];
...
const getSepatuPrompts = () => [
    `Professional product photography of the EXACT shoe/sandal from the source image. The setting is a glossy black surface (table or stairs). One shoe is worn on a right foot, which is slightly extended forward. The other shoe is held by a hand entering the frame, pointed towards the camera. Top view composition showing both clearly. Bright indoor lighting highlights small ornaments like flowers, ribbons, or characters. Dark background for high contrast. Lifestyle feel, casual and natural. 4K quality.`,
    `A high-end lifestyle catalog photo for the EXACT shoe/sandal from the source image. The product is displayed on a shiny black staircase. One sandal is on a foot, while the other is held up by a hand for the camera. Semi-top view angle. The lighting is bright and clear, making the product's details and decorations stand out. The background is dark and minimalist, creating a professional look. Sharp textures and vivid colors.`,
    `Cinematic product showcase of the EXACT shoe/sandal from the source image. The scene features a glossy black table. A person is wearing one shoe on their right foot and holding the other one towards the lens. Top-down perspective. Natural and casual lifestyle arrangement. The product's intricate details like ribbons or small characters are highly visible due to bright studio lighting. Dark background provides a striking contrast. Ultra-sharp 4K resolution.`,
    `A professional photoshoot of the EXACT shoe/sandal from the source image on a reflective black surface. Lifestyle composition: one shoe on a foot, the other held by a hand. Semi-top view. Bright indoor lighting ensures all small hiasan (ornaments) are prominent. The dark background makes the product stand out. Casual and natural atmosphere, capturing the essence of the design with high-fidelity textures.`
];

export const GoSepatu: React.FC = () => {
    const { t } = useLanguage();
    ...
    const handleGenerate = async () => {
        if (!sourceImage) {
            setError(t('goSepatu.errors.noImage'));
            return;
        }
        ...
        const prompts = getSepatuPrompts();
    ...
    const handleDownload = (url: string, index: number) => {
        ...
        link.href = url;
        link.download = `go-sepatu-${index + 1}-${Date.now()}.png`;
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goSepatu.page.title')}
                description={t('goSepatu.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                     ...
                     <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader 
                            step={3} 
                            title={t('goSepatu.sections.angle.title')}
                            description={t('goSepatu.sections.angle.description')}
                        />
                        <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="pt-4">
                        ...
                        >
                            {isGenerating ? (
                                ...
                                <span className="tracking-wide">{t('goSepatu.generateButton')}</span>
                            )}
                        </button>
                ...
                <section className="flex-1 w-full bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[500px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Go Sepatu Gallery</h3>
