...
const ANGLE_OPTIONS: { id: Angle; nameKey: string; prompt: string; }[] = [
    { id: 'eye-level', nameKey: 'goSofaV2.angles.eyeLevel', prompt: 'Eye-level shot,' },
    { id: 'high-angle', nameKey: 'goSofaV2.angles.highAngle', prompt: 'Top view or semi-top view composition from above,' },
    { id: 'close-up', nameKey: 'goSofaV2.angles.closeUp', prompt: 'Detailed close-up shot,' },
    { id: 'detail', nameKey: 'goSofaV2.angles.detail', prompt: 'Extreme macro photography, focusing on fine fabric textures and sharp details,' },
    { id: 'dutch-angle', nameKey: 'goSofaV2.angles.dutchAngle', prompt: 'Dutch angle shot,' }
];
...
const getSofaV2Prompts = () => [
    `Professional product photography of the EXACT clothing/product from the source image. The product is neatly placed on a wooden hanger, lying on a light-colored (off-white or cream) sofa. Top view or semi-top view composition. Artistically placed fashion magazines are scattered around the product. Natural sunlight from a window creates soft, elegant shadows on the sofa surface. Warm, minimalist, and elegant aesthetic. High-quality lifestyle photography with sharp fabric textures. Clean and aesthetic vibe. 4K quality.`,
    `A high-end lifestyle catalog photo for the EXACT product from the source image. The item is on a hanger, resting on a bright, light-toned sofa. Semi-top view angle. Fashion magazine props are arranged artistically nearby. The lighting is natural and warm, coming from a side window, casting gentle shadows. Minimalist and clean composition. The fabric details are ultra-sharp and clear. Professional studio quality.`,
    `Cinematic product showcase of the EXACT item from the source image. The product is displayed on a hanger atop a light-colored, minimal sofa. Top-down perspective. Natural window lighting creates a soft, cozy atmosphere with delicate shadows. Fashion magazines are used as aesthetic props. The overall look is warm, elegant, and modern. High-fidelity textures and sharp focus. Ultra-sharp 4K resolution.`,
    `A professional photoshoot of the EXACT product from the source image on a light-colored sofa. The product is on a hanger, neatly arranged. Lifestyle composition with fashion magazines as artistic props. Top view. Natural indoor lighting ensures all fabric details are prominent and elegant. The bright sofa background makes the product stand out. Casual yet sophisticated atmosphere, capturing the essence of the design with high-fidelity textures.`
];

export const GoSofaV2: React.FC = () => {
    const { t } = useLanguage();
    ...
    const handleGenerate = async () => {
        if (!sourceImage) {
            setError(t('goSofaV2.errors.noImage'));
            return;
        }
        ...
        const prompts = getSofaV2Prompts();
    ...
    const handleDownload = (url: string, index: number) => {
        ...
        link.href = url;
        link.download = `go-sofa-v2-${index + 1}-${Date.now()}.png`;
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goSofaV2.page.title')}
                description={t('goSofaV2.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                     ...
                     <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader 
                            step={3} 
                            title={t('goSofaV2.sections.angle.title')}
                            description={t('goSofaV2.sections.angle.description')}
                        />
                        <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="pt-4">
                        ...
                        >
                            {isGenerating ? (
                                ...
                                <span className="tracking-wide">{t('goSofaV2.generateButton')}</span>
                            )}
                        </button>
                ...
                <section className="flex-1 w-full bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[500px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Go Sofa v2 Gallery</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">
                                {isGenerating ? `Mengerjakan...` : `4 Variasi Foto Produk di Sofa Terang`}
                            </p>
                        </div>
                        {isGenerating && (
