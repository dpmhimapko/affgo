...
const ANGLE_OPTIONS: { id: Angle; nameKey: string; prompt: string; }[] = [
    { id: 'eye-level', nameKey: 'goHanger.angles.eyeLevel', prompt: 'Eye-level shot, slightly facing forward,' },
    { id: 'high-angle', nameKey: 'goHanger.angles.highAngle', prompt: 'Top view composition from above,' },
    { id: 'close-up', nameKey: 'goHanger.angles.closeUp', prompt: 'Detailed close-up shot,' },
    { id: 'detail', nameKey: 'goHanger.angles.detail', prompt: 'Extreme macro photography, focusing on fabric textures and sharp details,' },
    { id: 'dutch-angle', nameKey: 'goHanger.angles.dutchAngle', prompt: 'Dutch angle shot,' }
];
...
const getHangerPrompts = () => [
    `Professional product photography of the EXACT clothing/product from the source image. The product is hanging on a wooden hanger on a minimalist metal rack. A hand is entering the frame, gently holding one sleeve of the garment to create a natural and interactive look. Eye-level angle slightly facing forward. Minimalist room background with light wood floor and white curtains. Props include other hangers on the rack and aesthetic fashion photos hanging on the wall. Natural window lighting creates soft, warm shadows. Clean, modern, and aesthetic fashion studio vibe. High-quality lifestyle photography with sharp fabric textures. 4K quality.`,
    `A high-end lifestyle catalog photo for the EXACT clothing from the source image. The garment is on a hanger, suspended from a sleek metal rack. A hand is visible, adjusting or holding the sleeve for a natural touch. Eye-level perspective. The setting is a bright, minimalist studio with light wood flooring and white drapes. Fashion studio props like extra hangers and wall-mounted aesthetic photos are present. Natural side lighting from a window casts gentle shadows. Minimalist and clean composition. Ultra-sharp fabric details.`,
    `Cinematic product showcase of the EXACT item from the source image. The clothing is displayed on a hanger on a metal rack in a modern fashion studio. A hand is holding the sleeve, adding a lifestyle element. Eye-level shot. Natural window lighting creates a soft, cozy atmosphere with delicate shadows on the light wood floor. Aesthetic fashion photography prints are hanging in the background. The overall look is warm, elegant, and modern. High-fidelity textures and sharp focus. Ultra-sharp 4K resolution.`,
    `A professional photoshoot of the EXACT product from the source image. The item is on a hanger, hanging from a metal rack. One sleeve is held by a hand for an interactive feel. Eye-level angle. Minimalist room with light wood flooring and white curtains. Aesthetic fashion studio props like other hangers and wall photos are visible. Natural indoor lighting ensures all fabric details are prominent and elegant. Casual yet sophisticated atmosphere, capturing the essence of the design with high-fidelity textures.`
];

export const GoHanger: React.FC = () => {
    const { t } = useLanguage();
    ...
    const handleGenerate = async () => {
        if (!sourceImage) {
            setError(t('goHanger.errors.noImage'));
            return;
        }
        ...
        const prompts = getHangerPrompts();
    ...
    const handleDownload = (url: string, index: number) => {
        ...
        link.href = url;
        link.download = `go-hanger-${index + 1}-${Date.now()}.png`;
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goHanger.page.title')}
                description={t('goHanger.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                     ...
                     <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader 
                            step={3} 
                            title={t('goHanger.sections.angle.title')}
                            description={t('goHanger.sections.angle.description')}
                        />
                        <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="pt-4">
                        ...
                        >
                            {isGenerating ? (
                                ...
                                <span className="tracking-wide">{t('goHanger.generateButton')}</span>
                            )}
                        </button>
                ...
                <section className="flex-1 w-full bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[500px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Go Hanger Gallery</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">
                                {isGenerating ? `Mengerjakan...` : `4 Variasi Foto Produk di Rak Hanger`}
                            </p>
                        </div>
                        {isGenerating && (
