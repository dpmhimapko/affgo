...
const ANGLE_OPTIONS: { id: Angle; nameKey: string; prompt: string; }[] = [
    { id: 'eye-level', nameKey: 'goSofa.angles.eyeLevel', prompt: 'Eye-level shot,' },
    { id: 'high-angle', nameKey: 'goSofa.angles.highAngle', prompt: 'Flatlay or semi-top view composition,' },
    { id: 'close-up', nameKey: 'goSofa.angles.closeUp', prompt: 'Detailed close-up shot,' },
    { id: 'detail', nameKey: 'goSofa.angles.detail', prompt: 'Extreme macro photography, focusing on fine textures and intricate product details,' },
    { id: 'dutch-angle', nameKey: 'goSofa.angles.dutchAngle', prompt: 'Dutch angle shot,' }
];
...
const getSofaPrompts = () => [
    `Professional product photography of the EXACT product from the source image lying on a beige linen sofa in a warm and cozy room. Artistically placed around the product are stylish glasses, a canvas tote bag, and a fashion magazine. Natural sunlight from a window creates soft, elegant shadows, enhancing the cozy aesthetic. The background features textures of knit fabric and light wood flooring. Cinematic lighting with a soft focus effect. High-quality 4K resolution.`,
    `A high-end lifestyle catalog photo. The EXACT product from the source image is placed centrally on a textured beige linen sofa. The scene is decorated with a canvas tote bag, a pair of trendy glasses, and an open fashion magazine for an artistic touch. Soft window light casts gentle shadows across the sofa and the light wooden floor in the background. A cozy knit throw is visible nearby. Professional lighting, soft bokeh, and ultra-sharp details.`,
    `Cinematic flatlay photoshoot of the EXACT product from the source image on a premium beige linen sofa. The setting includes a canvas tote bag, stylish eyewear, and a fashion magazine arranged artistically. Warm natural light streams in, creating a serene and cozy atmosphere with soft shadows. Background shows light wood floor and knit fabric textures. High-fidelity textures, professional studio quality, and natural aesthetic.`,
    `A professional product display on a soft beige linen sofa in a modern cozy interior. The EXACT product from the source image is surrounded by a canvas tote bag, fashion magazine, and stylish glasses. Natural sunlight creates beautiful soft shadows on the sofa and the light wood floor. The atmosphere is warm and inviting. Ultra-sharp 4K photography with cinematic lighting and a soft focus background.`
];

export const GoSofa: React.FC = () => {
    const { t } = useLanguage();
    ...
    const handleGenerate = async () => {
        if (!sourceImage) {
            setError(t('goSofa.errors.noImage'));
            return;
        }
        ...
        for (let i = 0; i < totalImages; i++) {
            ...
            try {
                const fullPrompt = `ULTRA-REALISTIC COZY PHOTOGRAPHY: ${selectedAnglePrompt} ${prompts[i]} ${negativePrompt}`;
    ...
    const handleDownload = (url: string, index: number) => {
        ...
        link.href = url;
        link.download = `go-sofa-${index + 1}-${Date.now()}.png`;
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goSofa.page.title')}
                description={t('goSofa.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                     ...
                     <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader 
                            step={3} 
                            title={t('goSofa.sections.angle.title')}
                            description={t('goSofa.sections.angle.description')}
                        />
                        <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="pt-4">
                        ...
                        >
                            {isGenerating ? (
                                ...
                                <span className="tracking-wide">{t('goSofa.generateButton')}</span>
                            )}
                        </button>
                ...
                <section className="flex-1 w-full bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[500px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Go Sofa Gallery</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">
                                {isGenerating ? `Mengerjakan...` : `4 Variasi Foto Sofa Linen Beige & Fashion Props`}
                            </p>
                        </div>
                        {isGenerating && (
