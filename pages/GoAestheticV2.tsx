...
import { Download as DownloadIcon, Eye as ZoomIcon, Square as SquareIcon, RectangleHorizontal as RectangleHorizontalIcon, RectangleVertical as RectangleVerticalIcon, RefreshCw, Clock, Image as ImageIcon, User } from '../components/icons/LucideIcons';
import { ZoomModal } from '../components/ZoomModal';
import { PromoCard } from '../components/PromoCard';

type AspectRatio = '1:1' | '3:4' | '9:16' | '16:9';
type HandType = 'auto' | 'female' | 'male';

const HAND_OPTIONS: { id: HandType; nameKey: string; prompt: string; }[] = [
    { id: 'auto', nameKey: 'goAestheticV2.hands.auto', prompt: 'A human hand' },
    { id: 'female', nameKey: 'goAestheticV2.hands.female', prompt: 'A feminine, clean hand' },
    { id: 'male', nameKey: 'goAestheticV2.hands.male', prompt: 'A masculine hand' }
];
...
// V2 Prompts: Focus on Hand-held review style with the updated room setup
const AESTHETIC_V2_PROMPTS = [
    "A first-person POV shot of a [HAND] holding the product precisely in the center of the frame, review style. Background is a soft grey fur carpet. A large modern mirror leans against the wall, and a green potted plant sits nearby on a white shelf. Ambient teal LED strips glow along the baseboards. The lighting is extremely bright, professional, and clear, with elegant purple light accents.",
    "Cinematic close-up of a [HAND] presenting the product to the viewer, positioned in the center of the screen. Modern aesthetic room background with a plush grey fur rug and a large mirror reflecting the space. A white stand holds a vibrant green plant. Brilliant teal LED strips glow along the wall base. Exceptionally bright studio lighting creates sharp textures and a professional high-key look.",
    "POV product showcase. A [HAND] is holding the item precisely in the center of the frame against a modern studio backdrop featuring a soft grey fur carpet. The background includes a stylish mirror and a green potted plant. Teal LED ambient lighting glows at the edges. Sharp focus on the product, very bright professional photography lighting, and subtle purple glow effects.",
    "Editorial reviewer shot. [HAND] is holding and displaying the product in the center of the frame in a premium modern room. Behind it, a mirror and a green plant in a pot are visible against the wall. Grey plush rug on the floor. Striking teal LED lighting and powerful, bright studio lights creating a luxury commercial feel."
];

export const GoAestheticV2: React.FC = () => {
    const { t } = useLanguage();
    
    // State
    const [sourceImage, setSourceImage] = useState<UploadedImage | null>(null);
    const [aspectRatio, setAspectRatio] = useState<AspectRatio>('3:4');
    const [handType, setHandType] = useState<HandType>('auto');
    ...
    const handleGenerate = async () => {
        ...
        const initialResults: PhotoshootResult[] = AESTHETIC_V2_PROMPTS.map((_, i) => ({
            id: i,
            status: 'waiting'
        }));
        setResults(initialResults);

        const totalImages = AESTHETIC_V2_PROMPTS.length;
        ...
        const selectedHandPrompt = HAND_OPTIONS.find(h => h.id === handType)?.prompt || 'A hand';
        ...
        for (let i = 0; i < totalImages; i++) {
            ...
            try {
                // Replace placeholder with selected hand type
                const basePrompt = AESTHETIC_V2_PROMPTS[i].replace('[HAND]', selectedHandPrompt);
                const fullPrompt = `ULTRA-REALISTIC POV PHOTOGRAPHY: ${basePrompt} ${negativePrompt}`;
    ...
    const handleDownload = (url: string, index: number) => {
        ...
        link.href = url;
        link.download = `go-aesthetic-v2-${index + 1}-${Date.now()}.png`;
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goAestheticV2.page.title')}
                description={t('goAestheticV2.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                     ...
                     <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader 
                            step={3} 
                            title={t('goAestheticV2.sections.hand.title')}
                            description={t('goAestheticV2.sections.hand.description')}
                        />
                        <div className="grid grid-cols-2 gap-2">
                            {HAND_OPTIONS.map(opt => (
                                <button 
                                    key={opt.id} 
                                    onClick={() => setHandType(opt.id)} 
                                    disabled={isGenerating}
                                    className={`p-3 rounded-xl flex items-center justify-center gap-2 border transition-all ${
                                        handType === opt.id 
                                        ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/20 border-indigo-500 dark:text-indigo-300 shadow-sm' 
                        ...
                        >
                            {isGenerating ? (
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="tracking-wide">Reviewing ({progress}/4)...</span>
                                </div>
                            ) : (
                                <span className="tracking-wide">{t('goAesthetic.generateButton')}</span>
                            )}
                        </button>
                ...
                <section className="flex-1 w-full bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[500px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Review Gallery</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">
                                {isGenerating ? `Mengerjakan...` : `4 Variasi Produk Dipegang Tangan`}
                            </p>
                        </div>
                        {isGenerating && (
                    ...
                    {!results && !isGenerating && (
                        <div className="flex flex-col items-center justify-center h-96 text-slate-400 dark:text-slate-500 text-center">
                            ...
                            <p className="text-sm font-bold opacity-70">Hasil foto review akan muncul di sini.</p>
                        </div>
                    )}
