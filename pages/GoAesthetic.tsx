...
// Updated prompts to match the new aesthetic: carpet, side table, Eiffel Tower, plant, and window shadows
const getAestheticPrompts = (carpetColor: CarpetColor) => [
    `The EXACT product from the source image is placed centrally on a thick ${carpetColor} shag carpet. In the background, to the left, there is a small white round side table with wooden legs. On the table, two books are stacked with a small bronze Eiffel Tower model on top. To the right, a vibrant green potted plant in a white ceramic pot sits on the carpet. The light grey wall behind features soft, cinematic shadows of a window frame. The lighting is warm and natural, as if from a nearby window, creating a cozy and premium aesthetic.`,
    `Cinematic product photography of the EXACT product from the source image lying on a plush ${carpetColor} carpet. The setting is a minimalist room with a light grey wall. A small white side table with wooden legs stands to the left, holding a stack of books and a miniature Eiffel Tower. A lush green plant in a white pot is on the right. Soft natural sunlight streams in, casting elegant window shadows on the wall. The atmosphere is calm, sophisticated, and highly aesthetic.`,
    `A professional display of the EXACT product from the source image on a soft ${carpetColor} carpet. Background elements include a small white round table with wooden legs on the left, topped with books and an Eiffel Tower figurine. A green potted plant is on the right. The scene is illuminated by soft window light, casting beautiful geometric shadows on the light grey wall. High-end, clean, and modern interior design style.`,
    `The EXACT product from the source image sitting on a premium ${carpetColor} shag rug in a modern aesthetic room. To the left, a small white side table with wooden legs holds books and a small Eiffel Tower model. To the right, a green plant in a white pot adds a touch of nature. The light grey wall in the background is decorated with soft window frame shadows. The lighting is soft and natural, creating a high-quality commercial look.`
];
...
export const GoAesthetic: React.FC = () => {
    ...
    const [carpetColor, setCarpetColor] = useState<CarpetColor>('grey');
    ...
    const handleGenerate = async () => {
        ...
        const prompts = getAestheticPrompts(carpetColor);
        ...
        for (let i = 0; i < totalImages; i++) {
            ...
            try {
                const fullPrompt = `ULTRA-REALISTIC PRODUCT PHOTOGRAPHY: ${selectedAnglePrompt} ${prompts[i]} ${negativePrompt}`;
    ...
    const handleDownload = (url: string, index: number) => {
        ...
        link.href = url;
        link.download = `go-aesthetic-${index + 1}-${Date.now()}.png`;
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goAesthetic.page.title')}
                description={t('goAesthetic.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader 
                            step={3} 
                            title="Warna Karpet"
                            description="Pilih warna karpet bulu."
                        />
                        <div className="grid grid-cols-2 gap-2">
                            {(['grey', 'white'] as CarpetColor[]).map(c => (
                                <button 
                                    key={c} 
                                    onClick={() => setCarpetColor(c)} 
                                    disabled={isGenerating}
                                    className={`p-3 rounded-xl flex items-center justify-center gap-2 border transition-all ${
                                        carpetColor === c 
                                        ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/20 border-indigo-500 dark:text-indigo-300 shadow-sm' 
                                ...
                                >
                                    <div className={`w-4 h-4 rounded-full border border-slate-300 ${c === 'grey' ? 'bg-slate-400' : 'bg-white'}`} />
                                    <span className="text-sm font-bold capitalize">{c === 'grey' ? 'Abu-abu' : 'Putih'}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <section className="flex-1 w-full bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[500px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Aesthetic Gallery</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">
                                {isGenerating ? `Mengerjakan...` : `4 Variasi Foto Karpet ${carpetColor === 'grey' ? 'Abu' : 'Putih'} & Window Shadows`}
                            </p>
                        </div>
                        {isGenerating && (
