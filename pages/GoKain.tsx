...
const ANGLE_OPTIONS: { id: Angle; nameKey: string; prompt: string; }[] = [
    { id: 'eye-level', nameKey: 'goKain.angles.eyeLevel', prompt: 'Eye-level shot,' },
    { id: 'high-angle', nameKey: 'goKain.angles.highAngle', prompt: 'Top view flatlay composition from directly above,' },
    { id: 'close-up', nameKey: 'goKain.angles.closeUp', prompt: 'Detailed close-up shot,' },
    { id: 'detail', nameKey: 'goKain.angles.detail', prompt: 'Extreme macro photography, focusing on fine fabric textures and sharp details,' },
    { id: 'dutch-angle', nameKey: 'goKain.angles.dutchAngle', prompt: 'Dutch angle shot,' }
];
...
const getKainPrompts = () => [
    `Professional flatlay photography of the EXACT clothing/product from the source image. The product is placed on a light-colored fabric surface, spread out naturally following its original shape but slightly messy for a realistic look. Top view angle. Artistically placed lifestyle props like stylish glasses, fashion magazines, headphones, and a sleek laptop are scattered around the product. Natural lighting from above creates soft, clean, and fresh shadows. Minimalist background with light tones. High-quality lifestyle photography with sharp fabric textures and fine folds. Modern and aesthetic vibe. 4K quality.`,
    `A high-end modern flatlay catalog photo for the EXACT product from the source image. The item is displayed on a bright, light-toned fabric background. Top view angle. Lifestyle props including a laptop, headphones, and fashion magazines are arranged artistically nearby. The lighting is natural and fresh, coming from above, casting gentle shadows. Minimalist and clean composition. The fabric details and folds are ultra-sharp and clear. Professional studio quality.`,
    `Cinematic flatlay showcase of the EXACT item from the source image. The product is laid out on a light-colored fabric in a modern minimalist setting. Top-down perspective. Natural overhead lighting creates a soft, clean atmosphere with delicate shadows. Lifestyle elements like glasses and magazines are used as aesthetic props. The overall look is fresh, elegant, and modern. High-fidelity textures and sharp focus on fabric folds. Ultra-sharp 4K resolution.`,
    `A professional flatlay photoshoot of the EXACT product from the source image on a light-colored fabric surface. Lifestyle composition with laptop, headphones, and magazines as artistic props. Top view. Natural indoor lighting from above ensures all fabric details and fine folds are prominent and fresh. The bright background makes the product stand out. Casual yet sophisticated modern atmosphere, capturing the essence of the design with high-fidelity textures.`
];

export const GoKain: React.FC = () => {
    const { t } = useLanguage();
    ...
    const handleGenerate = async () => {
        if (!sourceImage) {
            setError(t('goKain.errors.noImage'));
            return;
        }
        ...
        for (let i = 0; i < totalImages; i++) {
            ...
            try {
                const fullPrompt = `ULTRA-REALISTIC FLATLAY PHOTOGRAPHY: ${selectedAnglePrompt} ${prompts[i]} ${negativePrompt}`;
    ...
    const handleDownload = (url: string, index: number) => {
        ...
        link.href = url;
        link.download = `go-kain-${index + 1}-${Date.now()}.png`;
    ...
    return (
        <div className="w-full">
            <FeatureHeader
                title={t('goKain.page.title')}
                description={t('goKain.page.description')}
            />
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <aside className="w-full lg:w-[400px] flex-shrink-0 lg:sticky lg:top-8 space-y-6 bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                     ...
                     <div className="pt-6 border-t border-gray-100 dark:border-white/10">
                        <StepHeader 
                            step={3} 
                            title={t('goKain.sections.angle.title')}
                            description={t('goKain.sections.angle.description')}
                        />
                        <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="pt-4">
                        ...
                        >
                            {isGenerating ? (
                                ...
                                <span className="tracking-wide">{t('goKain.generateButton')}</span>
                            )}
                        </button>
                ...
                <section className="flex-1 w-full bg-white dark:bg-gray-900/50 backdrop-blur-2xl p-6 md:p-8 rounded-[2.5rem] border border-white/40 dark:border-white/5 shadow-sm min-h-[500px]">
                    <div className="flex items-center justify-between mb-8 border-b border-gray-100 dark:border-white/10 pb-6">
                        <div>
                            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Go Kain Gallery</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 font-medium">
                                {isGenerating ? `Mengerjakan...` : `4 Variasi Foto Flatlay Produk di Atas Kain`}
                            </p>
                        </div>
                        {isGenerating && (
