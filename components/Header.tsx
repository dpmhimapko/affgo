...
export const Header: React.FC<HeaderProps> = ({ onMenuClick }) => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full px-2 sm:px-4 pt-4 pb-2">
      <div className="max-w-[1380px] mx-auto bg-white border-4 border-cartoon-dark rounded-3xl shadow-cartoon-lg transition-all duration-300">
        <div className="flex items-center justify-between h-16 px-4 sm:px-6">
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-1 min-w-0">
             <button
              onClick={onMenuClick}
              className="p-2 md:hidden rounded-xl border-2 border-cartoon-dark bg-cartoon-yellow hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-cartoon-hover shadow-cartoon transition-all shrink-0"
              aria-label="Open menu"
            >
              <MenuIcon className="w-6 h-6" />
            </button>

            <div className="flex items-center gap-2 sm:gap-3 group cursor-pointer select-none min-w-0">
              <div className="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-cartoon-blue border-2 border-cartoon-dark shadow-cartoon group-hover:rotate-6 transition-transform shrink-0">
                <AppLogoIcon className="w-7 h-7 sm:w-8 sm:h-8 text-white" />
              </div>
              <div className="flex flex-col min-w-0">
                <span className="text-lg sm:text-2xl font-black tracking-tighter text-cartoon-dark leading-none italic uppercase whitespace-nowrap">
